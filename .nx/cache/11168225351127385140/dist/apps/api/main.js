...
let AuthController = class AuthController {
    ...
    async login(body) {
        // 1. Manually check if the user/password is correct
        // (This uses the 'validateUser' method we fixed in the Service)
        const user = await this.authService.validateUser(body.email, body.password);
        // 2. If no user found, throw error immediately
        if (!user) {
            throw new common_1.UnauthorizedException('Invalid credentials');
        }
        // 3. If valid, generate the JWT token
        return this.authService.login(user);
    }
    async register(body) {
        // 👇 We already built this logic in AuthService, so let's use it!
